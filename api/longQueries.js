...
module.exports = {
    ...
    'getFAQ': `SELECT faq.ID, faq.QUESTION, faq.CATEGORY, faq.ANSWER, users.NAME AS ORGAS_NAME, orgas.FUNCTION FROM faq JOIN orgas ON orgas.ID = faq.ANSWERED_BY 
    JOIN users ON users.ID = faq.ANSWERED_BY WHERE IS_PUBLIC = 1`,
    'getAllFAQ': `SELECT faq.*, users.NAME AS ORGAS_NAME, orgas.FUNCTION FROM faq JOIN orgas ON orgas.ID = faq.ANSWERED_BY 
